...
## When to Use
Apply this skill whenever creating a presentation, slide deck, or structured output intended for stakeholders. Always apply the active theme. Default theme: `mgo`.
...
#### Theme: `mgo` (DEFAULT — Monopoly GO! Data & Insights)

The official Scopely/Monopoly GO! presentation template. All decks use this
theme unless explicitly overridden. Design system reference:
`.knowledge/templates/design-system.md`

**Template file:** `.knowledge/templates/Copy of Monopoly GO! Data & Insights Deck Template - May 2025.pptx`
**Output format:** `.pptx` (PowerPoint) — load from template file via python-pptx

- **Title font:** Jost ExtraBold, 48pt (content) / 80pt (cover & dividers)
- **Section heading font:** Jost Bold, 26pt, #3B3447
- **Body font:** Quicksand, 22pt, #3B3447
- **Graph title font:** Jost, 20pt, #3B3447
- **Table font:** Proxima Nova, 20pt
- **Source line font:** Quicksand, 16pt
...
| Dark Purple | `#3B3447` | Primary text |
| Accent Gold | `#DBC585` | Highlights, accents |
| Light Purple | `#CE9AFF` | Secondary differentiation |
| Olive | `#625C45` | Tertiary accent |

**Secondary colors (contrast):**
| Color | Hex | Usage |
|-------|-----|-------|
| Warm Cream | `#F8F5E5` | Callout backgrounds |
| Soft Green | `#B3E4BF` | Subtle positive |
| Medium Gray | `#CACACA` | Borders, dividers |
| Off-White | `#F7F6F3` | Slide/card backgrounds |

**Chart color sequence:** `#3B3447`, `#DBC585`, `#CE9AFF`, `#63D200`, `#DF2728`, `#625C45`, `#CACACA`

**Slide background:** White (#FFFFFF)

**Traffic Light System (performance reads):**
- Red `#DF2728` — Preliminary
- Gold `#DBC585` — In progress
- Green `#63D200` — Complete

**Deck structures:** The template defines 5 deck types (Standard, Exploratory,
Performance Read, A/B Test, Measurement Framework). See design-system.md for
full section breakdowns.

**How to generate:** Use python-pptx, loading from the template file. Remove
template example slides, then build new slides using the layout patterns and
font specs from the design system. Save to `outputs/` as `.pptx`.
...
### MGO PowerPoint Layout Rules (python-pptx)

These are the ONLY two layouts to use when generating `.pptx` from the MGO
template. Using any other layout causes text overlap or missing brand marks.

**Cover slide:** `prs.slide_layouts[5]` (BLANK_2 on default master 0)
- Title: Jost ExtraBold 60pt, position x=6.36", y=6.28", w=18.0"
- Subtitle: Quicksand 22pt, position x=6.36", y=8.86", w=18.0"
- Brand mark: manually add `mgo_brand_mark.png` at x=17.4", y=0.5"
- No artwork or logo images on cover

**Content slides:** `prs.slide_masters[1].slide_layouts[36]` (TITLE_10 on master 1)
- Has BUILT-IN brand mark at x=17.0", y=0.4" — do NOT add manually
- Title: custom textbox, Jost ExtraBold 44pt, x=0.4", y=0.4", w=15.1"
- Body: Quicksand 18-20pt, x=0.4", y=2.0"
- Source line: Quicksand 14pt, x=0.4", y=10.6", color=#CACACA

**Critical access pattern:**
```python
cover_layout = prs.slide_layouts[5]                      # BLANK_2 (master 0)
content_layout = prs.slide_masters[1].slide_layouts[36]  # TITLE_10 (master 1 ONLY)
```

**Common mistake:** `prs.slide_master.slide_layouts[36]` raises IndexError
because `prs.slide_master` (singular, no index) returns master 0 which only
has 33 layouts. TITLE_10 is on master 1 which has 100 layouts. Always use
`prs.slide_masters[1]` (plural, with index 1).

### Chart Embedding in PPTX

- **Never stretch charts.** Preserve original aspect ratio — set only width
  via `add_picture()`, never both width AND height (distorts text).
- **White background (#FFFFFF)** on all charts — not beige.
- **Legends go BELOW the chart** (`bbox_to_anchor`), never inside where they
  overlap bars or lines.
- **Cream callout boxes** (#F8F5E5 bg, #DBC585 border) to highlight key data.
- **No text below 9pt** in embedded charts — must be readable at slide scale.
- Run `check_label_collisions(fix=True)` before saving.

### Image Placement Rules

- Content slides: brand mark ONLY (mgo_brand_mark.png, 2.11×0.50", top-right).
  TITLE_10 layout has this built in — do not add again.
- **Never** place `mgo_artwork.png` or `mgo_logo.png` on content slides — they
  are large decorative images that conflict with text.
- Layout 0 (TITLE) is BANNED — has large artwork causing text overlap.
- BLANK layouts must not be used as fallback for content slides.

### Automatic Theme Selection

When no `{{THEME}}` is explicitly passed, Deck Creator auto-selects the theme based on context:

| Condition | Default Theme | Output Format | Rationale |
|-----------|--------------|---------------|-----------|
| `{{THEME}}` explicitly provided | Use as-is | Per theme | Explicit override always wins |
| No override specified | `mgo` | `.pptx` | Default for all Scopely/MGO analysis deliverables |
| `{{CONTEXT}}` is "workshop" or "talk" | `analytics-dark` | Marp PDF | Dark themes project better in live settings |
| `{{FORMAT}}` is "marp" (no context) | `analytics` (light) | Marp PDF | Marp-specific deliverables |
