...
SELECT
  ...
  DATE_DIFF(DATE(a.f_event_completion_time), c.start_date, DAY) AS early_completer, 
  ...
  ac.percent_total_participation,
  ...
  COUNT(DISTINCT CASE WHEN f.v_f_user_rpt.is_active AND COALESCE(a.currency_sink, 0) <> 0 THEN f.user_id END) AS cohort_returned_players, 
  SUM(CASE WHEN f.v_f_user_rpt.is_active AND COALESCE(a.currency_sink, 0) <> 0 THEN f.v_f_user_rpt.f_iap_revenue END) as cohort_revenue, 
  COALESCE(SUM(a.session_length_minute), 0) AS session_length_minute,
  COALESCE(SUM(a.revenue), 0) AS revenue,
  COUNT(DISTINCT CASE WHEN f.v_f_user_rpt.is_active AND f.v_f_user_rpt.f_last_7d_active_days > f.v_f_user_rpt.f_next_7d_active_days THEN a.user_id END) AS cohort_lapsed_players, 
...
FROM (
  SELECT
    t1.user_id,
    t1.liveops_id,
    MAX(t1.snapshot_date) last_day_active, 
...
LEFT JOIN `dwh-prod-tophat.BIZ.v_f_user_standard_kpis` f
  ON a.user_id = f.user_id AND f.snapshot_date between a.last_day_active - 30 AND a.last_day_active + 30 
GROUP BY ALL
### I chose these two events randome, to limit the data. should iterate on multiple to make sure results are the same
HAVING liveops_id in ('CoopAquaPartners2024@07082025_SE_Coop_AquaPartners2025','CookingPartners2025@10072025_SE_Coop_CookingPartners2025')
