...
- **Default for all analysis output → `mgo` (PowerPoint .pptx).** When in doubt, use `mgo`.
- The `mgo` theme uses the official Monopoly GO! Data & Insights template
  (`.knowledge/templates/Copy of Monopoly GO! Data & Insights Deck Template - May 2025.pptx`).
  Read `.knowledge/templates/design-system.md` for the full specification
  (fonts, colors, layout positions, deck structures).
- Workshop/talk → `analytics-dark` (Marp PDF).
- Explicit {{THEME}} override always wins.
- Never default to Marp or Gamma when `mgo` is the active theme.
...
2. If {{THEME}} is not provided:
   - If {{CONTEXT}} is "workshop" or "talk" → default to `analytics-dark` (Marp)
   - If {{FORMAT}} is "marp" → default to `analytics` (light theme, Marp)
   - **Otherwise → default to `mgo` (PowerPoint .pptx)**

**When theme is `mgo`:** Skip Marp/Gamma output entirely. Instead, use python-pptx
to build slides from the official template. Read the design system reference at
`.knowledge/templates/design-system.md` for exact fonts, colors, positions, and
layout patterns. The output file is `.pptx`, not `.md`.

**Brand mark placement (REQUIRED for all content slides):**
Every content slide (except cover and section dividers) must include the MGO brand
mark in the top-right corner. Use the pre-extracted asset:
```python
BRAND_MARK = '.knowledge/templates/assets/mgo_brand_mark.png'
# Place on EVERY content slide — top-right
slide.shapes.add_picture(BRAND_MARK, Inches(17.4), Inches(0.5), Inches(2.11), Inches(0.50))
```
The cover slide uses the full MGO logo (`.knowledge/templates/assets/mgo_logo.png`)
at `Inches(9.4), Inches(1.93), Inches(7.4), Inches(3.7)` instead.
...
**Data callout boxes (REQUIRED for insight slides with charts):**
Every chart slide should include 1-2 callout boxes highlighting the key takeaway
from the chart. Use the `add_data_callout()` helper from the design system:
```python
# Example: callout box to the right of a chart
add_data_callout(slide, "-18% — Mobile conversion dropped after iOS 18",
                 left=Inches(14.5), top=Inches(4.0))
```
Place callouts adjacent to (not overlapping) the chart. Use Warm Cream (#F8F5E5)
background with Gold (#DBC585) left border. See design-system.md for the full
implementation. Maximum 2 callouts per slide.
...
**If theme is `mgo` (DEFAULT — PowerPoint .pptx):**

Build the deck using python-pptx, loading from the official template. Read
`.knowledge/templates/design-system.md` for the full specification.
...
TEMPLATE = '.knowledge/templates/Copy of Monopoly GO! Data & Insights Deck Template - May 2025.pptx'
prs = Presentation(TEMPLATE)

# STEP 1: Remove ALL template example slides (keep only the master/layouts)
...
# STEP 2: Build new slides using the design system spec
# Fonts: Jost ExtraBold (titles), Quicksand (body), Proxima Nova (tables)
# Colors: #3B3447 (text), #DBC585 (gold), #DF2728 (red), #63D200 (green)
# Title position: x=0.4", y=0.4", Jost ExtraBold 48pt
# Chart area: starts y=2.0"
# Supporting message: y=9.1", Quicksand 22pt
# Source line: y=10.6", Quicksand 16pt

# Use BLANK layout and place shapes manually for full control
blank_layout = prs.slide_layouts[25]  # BLANK

# For each slide in the outline, add a slide and place text boxes + images
# following the layout patterns from design-system.md
```

**Slide construction pattern for each slide type:**

| Slide Type | Title Font | Title Size | Body Font | Body Size | Chart Position |
|-----------|-----------|-----------|----------|----------|---------------|
| Cover | Jost ExtraBold | 80pt | Proxima Nova | 22pt | N/A |
| Section Divider | Jost ExtraBold | 80pt | — | — | N/A |
| Content (1 chart) | Jost ExtraBold | 48pt | Quicksand | 22pt | x=0.4", y=2.4", w=19", h=6.3" |
| Content (2 charts) | Jost ExtraBold | 48pt | Quicksand | 18pt | Left: x=0.4", Right: x=10.3", w=9.4" each |
| Exec Summary | Jost ExtraBold | 48pt | Quicksand | 22pt | N/A (text only) |
| Recommendations | Jost ExtraBold | 48pt | Quicksand | 22pt | N/A (table or text) |

**Chart embedding — ASPECT RATIO PRESERVATION (CRITICAL):**
Charts render at (10, 6) figsize / 150 DPI (~1500x900px = 5:3 aspect ratio).
When embedding charts, NEVER set both width AND height — this stretches the image
and distorts text, making it fat or tall and unreadable. Instead:

```python
from pptx.util import Inches, Emu
from PIL import Image

def add_chart_preserve_aspect(slide, chart_path, left, top, max_width, max_height):
    """Add a chart image to a slide while preserving its original aspect ratio.
    
    Sets the image to fit within (max_width, max_height) without distortion.
    The image is scaled down to the largest size that fits while keeping its
    natural proportions.
    """
    ...
    # Calculate dimensions that fit within the max box while preserving ratio
    fit_width = max_width
    ...
    pic = slide.shapes.add_picture(chart_path, left, top, fit_width, fit_height)
    return pic
```

Use `add_chart_preserve_aspect()` for ALL chart placements. Standard positions:
- Full-width chart: left=Inches(0.4), top=Inches(2.4), max_width=Inches(19.0), max_height=Inches(6.3)
- Two-chart layout: max_width=Inches(9.4), max_height=Inches(6.3) each

**NEVER call `add_picture(path, left, top, width, height)` directly for charts.**
Always use the aspect-ratio-preserving wrapper above. Text in stretched charts
becomes unreadable — fat letters, blurry annotations, distorted axis labels.

**Source line:** Add on every data slide at y=10.6", Quicksand 16pt.

Save as `outputs/deck_{{DATASET_NAME}}_{{DATE}}.pptx`
