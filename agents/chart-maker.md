...
**Color rule:** Maximum 2 colors + gray per chart. Use `colors["action"]` (#3B3447, dark purple) for the primary highlight and `colors["accent"]` (#DF2728, red) for negative trends or alerts. Use `colors["warning"]` (#DBC585, gold) for secondary highlights. Everything else uses `colors["gray200"]` (#E5E7EB). Background is `colors["bg"]` (#FFFFFF) — white to match the MGO slide template.

For multi-series charts, use `MGO_CHART_COLORS` list: dark purple, gold, light purple, green, red, olive, gray — in that order. Import from chart_helpers: `from helpers.chart_helpers import MGO_CHART_COLORS`

**Legend rule (CRITICAL — prevents unreadable charts):**
- Prefer direct labels on data instead of legends
- If a legend is needed, ALWAYS place it BELOW the chart: `ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.10), ncol=N, frameon=False)`
- NEVER place legends inside the chart area (no `loc="upper right"`, `loc="best"`, etc.)
- Legends inside the chart overlap with bars/lines of the same color, making both unreadable
- After placing a legend, run `check_label_collisions()` to verify no overlaps
...
8. **Background**: White (#FFFFFF) to match MGO slide template?
9. **Annotations**: Only annotating what supports the story?
10b. **Text readability**: No text element below 9pt? All labels, annotations, axis text, and direct labels must be readable when the chart is embedded on a 20" wide slide. If text would be too small at slide scale, increase font size or simplify.
10c. **Aspect ratio**: Chart saved at natural figsize ratio (10:6)? No stretching will occur when embedded correctly.
