...
COLORS = {
    # MGO brand palette (default) — matches .knowledge/templates/design-system.md
    "action":   "#3B3447",   # dark purple — primary series / highlight
    "accent":   "#DF2728",   # alert red — negative metrics, warnings
    "negative": "#DF2728",   # alert red
    "success":  "#63D200",   # success green
    "warning":  "#DBC585",   # accent gold — highlights, accents
    "danger":   "#DF2728",   # alert red
    "gray900":  "#3B3447",   # dark purple text (MGO primary text)
    "gray600":  "#625C45",   # olive brown — tertiary / muted text
    "gray400":  "#CACACA",   # medium gray — borders, dividers
    "gray200":  "#E5E7EB",   # light gray — background bars/lines
    "gray100":  "#F7F6F3",   # off-white — subtle fills
    "white":    "#FFFFFF",
    "bg":       "#FFFFFF",   # white background — matches MGO slide template
    # MGO chart color sequence (use in this order for multi-series charts)
    "mgo_1":    "#3B3447",   # dark purple — primary
    "mgo_2":    "#DBC585",   # gold — secondary
    "mgo_3":    "#CE9AFF",   # light purple — third
    "mgo_4":    "#63D200",   # green — fourth
    "mgo_5":    "#DF2728",   # red — fifth (or negative)
    "mgo_6":    "#625C45",   # olive — sixth
    "mgo_7":    "#CACACA",   # gray — baseline/reference
    # Semantic aliases
    "primary":  "#3B3447",   # dark purple — main data series
    "muted":    "#CACACA",   # medium gray — supporting elements
}

# MGO chart color sequence as a list (for multi-series charts)
MGO_CHART_COLORS = [
    "#3B3447",  # dark purple
    "#DBC585",  # gold
    "#CE9AFF",  # light purple
...
CHART_FIGSIZE = (10, 6)  # ~1500x900 @ 150 DPI — 5:3 aspect ratio for slide embedding
# CRITICAL: When embedding charts in PPTX slides, NEVER set both width AND height
# on add_picture(). This stretches the image and distorts all text. Always preserve
# the original aspect ratio by setting only width (let height auto-scale) or use
# the add_chart_preserve_aspect() helper from deck-creator.
...
def stacked_bar(ax, categories, layers, colors_map=None, highlight_layer=None,
    ...
    # Legend: ALWAYS positioned below chart to avoid occluding data.
    # NEVER place legends inside the chart area — they overlap bars/lines.
    ax.legend(fontsize=9, frameon=False, loc="upper center",
              bbox_to_anchor=(0.5, -0.10), ncol=min(len(layers), 5))
...
def grouped_bar(df, x_col, y_col, group_col, highlight_group=None,
    ...
    # Legend: ALWAYS below chart to prevent overlap with bars/data
    ax.legend(fontsize=9, frameon=False, loc="upper center",
              bbox_to_anchor=(0.5, -0.10), ncol=min(n_groups, 4))
