...
| **Background** | `#FFFFFF` | Chart and slide background — white (matches MGO template) |
...
**Theme integration:** When using the MGO theme (default), charts use white background (#FFFFFF) matching the slide template. Chart colors follow the MGO palette: #3B3447 (dark purple primary), #DBC585 (gold), #CE9AFF (purple), #63D200 (green), #DF2728 (red). For Marp themes, background derives from CSS tokens. This ensures charts feel native to the slide deck, not copy-pasted.
...
- [ ] **3D effects** — never
- [ ] **Background color** — always `#FFFFFF` (white, matches MGO slide template)
